...
“导入预设”默认使用“仅导入数值”：先复制你选择的基础预设，再把旧备份中精确匹配或可唯一确认的同语义参数套用到新副本，并直接切换到该副本；基础预设和导入前正在编辑的 Dashboard 都不会被改写，导入前可命名新预设。选择“布局和数值”也会额外创建命名预设；若文件含失效绑定，确认前会明确警告。歧义、失效或不兼容值会列出并跳过；模型选项未出现在当前列表时，会提示检查模型名称与相对路径，文件确实存在则刷新 ComfyUI 网页后重试。
