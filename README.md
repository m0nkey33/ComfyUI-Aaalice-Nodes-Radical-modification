...
搜索并按嵌套分类树或收藏夹筛选词条，然后选择任意数量；选择父分类会包含其全部后代，选择器使用完整路径区分同名子分类。列表可以把最近队列使用过的提示词排在前面。选择顺序即输出顺序。悬停已选词条可用滚轮或方向键调整权重（0–20）。可选的 `prefix_prompt` 输入最先输出，节点菜单可修改分隔符（默认 `, `）。安装 ComfyUI-Autocomplete-Aaalice 后，搜索框还会提供其标签与中文补全。
...
- 搜索同时接受 booru 风格标签查询和粘贴的提示词文本；Danbooru 提供最近完整自然日的日榜及滚动周榜/月榜，AI TAG 提供月榜。
- 打开“随机抽卡”后会在当前来源、查询、频道和分级范围内随机抽取图片；前端使用 Web Crypto 安全洗牌，并在当前节点会话内避免重复展示相同帖子，关闭模式或改变范围后重新开始。Danbooru 的 Rating 是免费元标签，`random:<limit>` 则会占用一个查询名额：0–1 个普通标签使用原生随机并覆盖完整匹配集，两个普通标签通过帖子 ID 游标在完整历史跨度内抽样，避免深页查询触发站点执行预算。随机结果继续遵循本机内容黑名单。
...
**词库**视图管理词条、任意层级的分类树、多归属收藏夹、标签和每条词条的一张预览图。分类支持搜索、折叠、拖拽与键盘跨层排序；安全删除会提升子分类并把直属词条设为未分类，也可显式删除整棵分支，但不会删除词条。已选词条可一次移动、导出或事务化删除，每条词条的完整提示词可一键复制。词库可将全部或当前筛选结果导出为带哈希资产的 v2 ZIP；现有平面 SQLite 词库、v1 ZIP 和旧版 `data.json + preview/` 会自动升级，旧分类名中的有效 `/` 路径会一次性转换为分类层级，同时保留原叶分类 ID、颜色和词条归属。传输上限 2 GiB，全程流式处理而非读入内存。
