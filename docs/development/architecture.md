...
| BooruGalleryNode | `node.properties.booruGalleryState`（查询上下文、逻辑页码、随机/选择模式、选择快照与 Dashboard 搜索展开状态）、节点自身 `mode`；Dashboard 值预设另以版本化整体 payload 保存该状态快照 | 单个控制器的搜索 Summary、随机会话已见集合、详情、当前请求、执行 payload，以及任意数量节点 / Dashboard Surface 投影；Dashboard 只读显示节点 mode，并独立投影双行紧凑工具栏、共享 `activeLabelOnly` 收缩 Switcher 与更小瀑布流卡片 | 任一投影 DOM、搜索结果副本、随机顺序、cursor、滚动像素、Hover、Dialog、凭据或缓存 |
...
4. 用户开关先在纯模型中规划同 Manager 级联和节点模式变化；现存来源组的环路、缺失目标、路径冲突或重叠组冲突会在写入前中止。已删除来源组遗留的规则继续保存在工作流中，以便同 Id 的组恢复后检查，但不会阻断其它现存组编辑联动。节点面和侧边栏都通过同一 runtime 提交模式；图事务完成后同步刷新同 graph 的全部已挂载 Manager 侧边栏投影，节点面则由合并后的 `graphChanged` 刷新。
...
4. SQLite 启动迁移通过列检查为旧 `categories` 增加 `parent_id`，并用 `library_metadata` 标记只执行一次的旧路径迁移：根分类名中的有效 `/` 路径生成缺失的中间节点，原分类 ID 作为叶节点保留，词条引用不重建；普通名称仍为根分类。创建、跨层移动、安全提升删除和整分支删除都在单一领域事务中校验父级、循环与目标索引，并连续归一化受影响兄弟顺序；词条在任何删除模式下只会变为未分类。
5. ZIP 只上传一次到有时效的磁盘暂存区，完成结构、分类树、路径、大小、图片和哈希预检后返回 token，再以 SQLite 事务应用冲突策略；应用失败时数据库回滚且暂存 token 保留到重试、显式放弃或到期，成功后才清理。归档 v2 保存 `parentId`；v1 ZIP 与旧 JSON 的有效斜杠分类路径先迁移为树，普通分类迁移为根，导出先生成有时效文件并由浏览器原生流式下载。
...
5. capability 控制 Rating、排行榜、直接跳页、认证、原图下载和收藏按钮。排行榜走适配器独立入口；Danbooru 日榜显式锚定最近已完成的自然日，周榜与月榜保持原站滚动周期；逻辑页码统一从 1 开始，远端 `page` / `pid` 转换不进入前端。Danbooru 的 Rating 属于不占用查询标签名额的免费元标签，始终随搜索请求发送；适配器仍对 Summary 执行同一分级过滤，防止上游异常结果泄漏。Gelbooru 的搜索、详情和标签分类必须使用官方 User ID / API Key；设置边界接受账户页复制的 `&api_key=…&user_id=…` 凭据片段并规范化保存，不能改动其它来源凭据。其 Rating 使用站点当前的 General、Sensitive、Questionable、Explicit；单选分级发送远端 metatag，多选分级因远端不支持同类 metatag OR 而在真实分页结果上本地过滤。Gelbooru 不写收藏。Safebooru 与 AI TAG 不显示账户收藏；AI TAG 直接使用公开搜索、月榜与作品详情 API，并从公开图片元数据生成 Prompt，不把它伪装成传统 Booru Rating/标签分类。AI TAG 列表只提供推导缩略图时保留资源目录大小写；若首图并非 `_p0`，卡片仅在图片失败时按需请求详情恢复真实首图，不把逐帖详情请求恢复到搜索主路径。所有来源都不使用 Cookie、HTML 会话或验证码兼容层。
...
7. 随机模式只把开关写入 `booruGalleryState`；已见 `source + postId` 集合由单个前端节点会话拥有，并以来源、查询、频道、周期和 Rating 组成作用域，退出模式或作用域变化时释放。随机 API 不接收顺序 cursor、不写搜索结果缓存，前端请求同时使用 `cache: "no-store"`。Danbooru 的 Rating 是免费元标签，`random:<limit>` 会占用一个公开查询名额：0–1 个普通标签使用原生随机覆盖完整匹配集，不执行 `order:random` 全量排序；两个普通标签通过最新页和 `a0` ID 游标探测匹配集首尾 ID，范围缓存键包含来源、规范化查询、规范化 Rating 与账户身份，再用 `secrets.randbelow()` 在 ID 区间选择阈值并通过 `b<ID>` 游标获取真实批次。该路径不调用计数端点、不使用 OFFSET 深页，覆盖完整历史 ID 跨度且每次缓存命中只请求一个结果批次；ID 区间采样是按历史跨度的近似随机，不保证稀疏匹配集逐帖严格等概率。Gelbooru / Safebooru 搜索使用来源原生随机排序，AI TAG 以只缓存总数的随机页采样覆盖全集，前端再用 Web Crypto 拒绝采样驱动 Fisher–Yates 洗牌并做稳定身份去重。适配器继续先执行本机黑名单与必要的本地 Rating 过滤，客户端只把实际展示项记入已见集合；唯一结果耗尽采用有限空批次预算，不以无限翻页维持表面成功。
...
12. 画布绑定高亮由 `js/lib/canvas_control_binding_highlight.js` 维护，Classic widget 绘制兼容层独立位于 `js/lib/canvas_widget_marker.js`：现代 native/promoted widget 使用 `getOutlineColor()`，拥有 legacy/custom `draw()` 的 widget 还会在自身绘制后直接补画紫框，不能假设其消费现代颜色钩子。富 DOM widget 的紫框必须复用宿主 `margin` 与 `computedHeight` 几何，和 Classic `DomWidgets.vue` 的实际挂载矩形一致，不能把传入 `draw()` 的固定 `NODE_WIDGET_HEIGHT` 或通用 native margin 当作真实边界。新版 Subgraph 的 store-backed native 投影会在每帧由 `toConcreteWidget()` 创建瞬时绘制实例，不能包装投影对象后期待包装被复制；兼容层只为这类无法直接标记的已绑定投影包装其宿主节点 `drawWidgets()`，在原绘制结束后按缓存目标集合补框。逐帧路径不扫描 graph、Dashboard 或其它 widget；宿主替换 widget `draw()` 或 node `drawWidgets()` 后由下一次结构同步重新包装，解绑时只恢复仍由本包持有的方法。标记集合新增、替换或移除后必须调用 ComfyUI `LGraphCanvas.setDirty(true, true)`，因为安装包装不会自动重绘已经保留的画布帧。含 `PreviewAny` / `PreviewImage` 的子图首次恢复也遵守该契约，进入/退出子图触发的重绘不能作为修复手段。高亮解析只依赖绑定的 node/widget 身份，与控件实时值无关，因此按（Dashboard 模型引用 + 图结构签名）备忘解析结果，结构未变时同步只重放 DOM/标记差异；备忘录不按当前图过滤，图导航后重新过滤，`beforeConfigureGraph` 必须调 `invalidateCanvasControlBindingResolution()` 防止同签名重载后命中旧节点对象。Nodes 2.0 的行映射候选列表读取新协议投影 widget 的响应式 store 访问器，只能在真实重解析时刷新，DOM mutation 触发的行重映射必须复用缓存。
