...
- Search accepts booru-style tag queries and pasted prompt-style text; Danbooru exposes the latest completed daily ranking plus rolling weekly/monthly rankings, AI TAG its monthly ranking.
- Enable **Random draw** to sample the current source, query, feed, and ratings. The frontend uses Web Crypto to shuffle safely and avoids replaying the same post within the current node session; leaving the mode or changing that scope starts a fresh session. Danbooru ratings are free metatags, while `random:<limit>` consumes one query slot: searches with zero or one ordinary tag use native random sampling across the complete matching set, while two-tag searches sample the full historical span through post-ID cursors so deep pages cannot exhaust the site's execution budget. Random results still respect the local content blacklist.
